...
function randomize(loadFromUrl) {
    ...
    if (loadFromUrl && urlParams.has('loadout')) {
        // Load loadout from URL params;
        try {
            generated = JSON.parse(urlParams.get('loadout'));
            appendRandoms(generated);
        } catch (error) {
            console.error('Could not parse loadout from URL params:', error);
            generated = null;
        }
    }
    if (!generated) {
        generated = generateWeapons();

        // Store generated loadout to URL params.
        urlParams = new URLSearchParams();
        urlParams.set('loadout', JSON.stringify(generated));
        document.location.search = urlParams;
    }

    var totalCost = generated.totalCost;
